...
class Philter:
    ...
    def init_patterns(self):
        ...
        for i, pattern in enumerate(self.patterns):
            ...
            if pattern["type"] in require_files and not os.path.exists(
                pattern["filepath"]
            ):
    ...
    def map_coordinates(self, allowed_filetypes=set(["txt", "ano"])):
        ...
        for root, dirs, files in os.walk(in_path):
            for f in files:
                ...
                for i, pat in enumerate(self.patterns):
                    if pat["type"] == "regex":
                        ...
                        self.map_regex_context(
                            filename=filename, text=txt, pattern_index=i
                        )
                        ...
                        raise Exception(
                            "Error, pattern type not supported: ", pat["type"]
                        )
                ...
                self.full_exclude_map[filename] = self.include_map.get_complement(
                    filename, txt
                )

                for phi_type in self.phi_type_list:
                    for start, stop in self.phi_type_dict[phi_type][0].filecoords(
                        filename
                    ):
    ...
    def map_regex(
        self, filename="", text="", pattern_index=-1, pre_process=r"[^a-zA-Z0-9]"
    ):
    ...
    def map_regex_context(
        ...
        try:
            ...
            warnings.warn(
                "deprecated missing context_filter field in filter "
                + str(pattern_index)
        ...
        if context_filter == "all":
            ...
            full_exclude_map_coordinates = self.patterns[context_filter_pattern_index][
                "coordinate_map"
            ]
            full_exclude_map = {}
        ...
        for m in matches:
            ...
            if (
                (context == "left" and phi_left == True)
    ...
    def map_pos(
        ...
        if "pos" not in self.patterns[pattern_index]:
            raise Exception(
                "Mapping POS must include parts of speech",
                pattern_index,
                "pattern length",
                len(patterns),
            )
            ...
            if pattern["type"] != "regex_context":
                if exclude:
                    if not self.include_map.does_overlap(filename, start, stop):
                        ...
                        self.phi_type_dict[phi_type][0].add_extend(
                            filename, start, stop
                        )
                    ...
                    if not self.exclude_map.does_overlap(filename, start, stop):
                        ...
                        self.data_all_files[filename]["non-phi"].append(
                            {
                                "start": start,
                                "stop": stop,
                                "word": txt[start:stop],
    ...
    def eval(
        ...
        for root, dirs, files in os.walk(in_path):
            for f in files:
                ...
                anno_filename = os.path.join(
                    anno_path, "".join(f.split(".")[0]) + anno_suffix
                )
        ...
        # Conext
        with open(
            os.path.join(self.eval_outpath, "fn_tags_context.txt"), "w"
        ) as fn_file:
            fn_file.write(
                "key"
                + "|"
        ...
        with open(
            os.path.join(self.eval_outpath, "fp_tags_context.txt"), "w"
        ) as fp_file:
            fp_file.write(
                "key"
                + "|"
            ...
            for f in files:
                ...
                if len(anno_suffix) > 0:
                    if not os.path.exists(
                        os.path.join(anno_folder, f.split(".")[0] + anno_suffix)
                    ):
                        print(
                            "FILE DOESNT EXIST",
                            os.path.join(anno_folder, f.split(".")[0] + anno_suffix),
